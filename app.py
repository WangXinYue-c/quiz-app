...
10. **阅读理解题特别处理**：如果遇到阅读理解题（英语阅读、语文阅读等），必须将完整的阅读语料（文章段落）包含在每道题的stem字段中。格式为：先放完整阅读语料，然后换行放问题。例如：stem = "阅读以下材料回答问题：\\n\\n[完整阅读材料]\\n\\n题目：根据上述材料，以下哪项是正确的？"。每道阅读题的stem都必须包含完整的阅读语料，不能省略或简写为"同上"或"参考上文"。
11. 如果文本中包含图表、图片的描述文字（如"如下图所示"），将相关描述文字保留在题干中
12. 对于包含表格数据的题目，将表格内容用文字描述保留在题干中'''
    # 文本太长的话分段处理（每次约4000字，降低内存峰值）
    chunk_size = 4000
    chunks = []
